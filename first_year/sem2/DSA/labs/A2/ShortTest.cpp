...
void testIncreasePriority()
{
	...
	PriorityQueue pq(rel);
	assert(pq.increasePriority(1, 1) == -1);

	// add 5, 3, 10, 2, 12 -> result: 2, 3, 5, 10, 12
	pq.push(5, 5);
	pq.push(3, 3);
	pq.push(10, 10);
	pq.push(2, 2);
	...
	try {
		pq.increasePriority(3, 5); // new priority is less than the old one
	}
	catch (const exception& e) {
		assert(true);
	}

	assert(pq.increasePriority(11, 11) == -1); // element does not exist
	assert(pq.increasePriority(12, 1) == 12); // returns the old priority and now we have 12, 2, 3, 5, 10
