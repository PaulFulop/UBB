...
#True if all the points in the combination are collinear
...
def determine_subsets(n: int, points: list):
    ...
    for p in range(3, n + 1):
        ...
        backtracking_recursive(n, p, 0, combination, points, found) #generate all subsets of length p included in {1, 2, ..., n}
...
#O(n * combinations n, k) -> we generate all combinations of length k and verify if the points are collinear everytime => n + ... + n -> n * (combinations n, k) operations 
def backtracking_recursive(n:int, k:int, start_index:int, combination: list, points: list, found_collinear:list):
    if k == 0:
        display(combination, points) #O(n)
        found_collinear[0] = True
        return
    
    #O(combinations n, k)
    for i in range(start_index, n - k + 1): #n - k + 1 prevents cases where adding more elements would be impossible
        if len(combination) < 2 or collinear_points(combination + [i], points) == True:
            combination.append(i)
            backtracking_recursive(n, k - 1, i + 1, combination, points, found_collinear)
            combination.pop()
